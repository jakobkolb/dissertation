...
\begin{sidewaystable}
	...
	\begin{tabular}{r|l|c|l}
          \makecell[l]{Symbol} & \makecell[l]{Default \\ Value} & \rotatebox[origin=c]{90}{Unit} & \makecell[l]{Parameter \\ Description} \\\hline
          $b_c$ & 22.94444 & $\mathrm{people}^{-\alpha} ~ \mathrm{U.S.\$}^{1-\beta-\gamma}$ & \multirow{2}{*}{\makecell[l]{total factor productivity (TFP) in \\the clean and dirty sector. }} \\ \hhline{---~}
          $b_d$ & 1844 & $\mathrm{people}^{-\alpha} ~ \mathrm{U.S.\$}^{1-\beta}$& \\ \hline
                ...
                $L$ & $3.38 \cdot 10 ^{9}$ & billion people & Total labor. \\ \hline
                ...
                $e$ & 4505 & \$/toe & \\
                & & & \\ \hline
                $G_0$ & $15.8 \cdot 10^5$ & Mtoe & \multirow{2}{*}{\makecell[l]{Initial fossil resource stock. Set according to \eqref{eq:resource_depletion_time} \\ depending on $b_R$, $e$, $b_d$,  $\delta$, $s$ and $L$.}}\\
                & & & \\ \hline
...
\begin{wrapfigure}[20]{i}{.55 \textwidth}
	\vspace{-.4 cm}
        ...
        \caption{Energy usage divided by dirty (coal, oil, gas) and clean (hydro, nuclear, other renewables). The dashed line indicates the fraction of clean energy consumption. Data from \cite{dudley2019bp}.\label{fig:energy_data}}
\end{wrapfigure}
...
\textit{Total labor $L$} is taken from wold bank data \footnote{see \url{https://data.worldbank.org/indicator/SL.TLF.TOTL.IN}} which estimates it at 3.38 billion people for 2016.\\
...
\begin{wrapfigure}[21]{i}{.55 \textwidth}
	\vspace{-.4 cm}
        %\hspace{-1.5 cm}
        \includegraphics[width = .65 \textwidth]{./figures/fossil_resource_per_year_extrapolated.pdf}
...
\textit{I estimate the initial value of the knowledge stock in the clean sector $C$} for the base year from the time series of production in the clean sector that is approximated equivalently to \ref{eq:energy_intensity_estimate} as world GDP times the fraction of clean energy consumption for the years of 1965-2010. I use the discrete version of \eqref{eq:learning_by_doing}:
\begin{equation}
...
\begin{equation}
  G_0 \approx G(t=2010) + \sum_{t=1948}^{2010} R(t) = 15.8 \cdot 10^{5} ~ \mathrm{Mtoe}.
...
The I estimate the parameters of the fossil resource cost function $b_R$ and $\mu$ from historical energy price and fossil resource consumption data. I use the historical oil price\footnote{I use the yearly average oil price for different types of crude oil according to \url{https://www.statista.com/statistics/262858/change-in-opec-crude-oil-prices-since-1960/}} as proxy for the fossil resource price $p_R$ as historical data for prices for different types of fossil energy sources \cite{owidfossilfuels} shows that prices are correlated and of the same order of magnitude. I create a time series of the fossil resource stock $G(t)$ from $G_0$ as estimated in eq. \ref{eq:initial_fossil_resource_approximation} and the time series of fossil resource use in fig. \ref{fig:historical_resource_use} like
...
Now, if you think that these units look weird, you are not alone. Maybe comment here on the discourse about this and on possible ways to circumvent the problem all together. \\
