...
\begin{wrapfigure}[16]{i}{.55 \textwidth}
	\vspace{-1.5 cm}
...
\begin{align}
\dot{K}_c &= s b_c L^{\alpha_c} K_c^{\beta_c} C^\gamma - \delta K_c \\
\dot{C} &= b_c L^\alpha_c K_c^{\beta_c} C^\gamma - \chi C
	\label{eq:full_clean_ca}
\end{align}
Assuming that $\alpha_c + \beta_c$ = 1, with equal elasticities for capital and labor e.g. $\alpha_c = \beta_c$, the stationary points of the system are 
\begin{equation}
...
\begin{equation}
  \lambda_{1,2} = \frac{1}{4} \left(2 \chi (\gamma-1) -\delta \pm \sqrt{4 (3 \gamma -1) \delta  \chi +4 (\gamma -1)^2 \chi ^2+\delta ^2}\right).
	\label{eq:learning_eigenvalues}
\end{equation}
Assuming capital and knowledge depreciate at similar rates, we can substitute $\chi \rightarrow \delta + \varepsilon$ and expand in powers of epsilon to see under which conditions the stationary point $(K^*_c,C^*)$ is stable. This results in:
\begin{align}
  \lambda_1 &= - \delta - \frac{1}{2\gamma+1}\varepsilon + O\left[ \varepsilon^2 \right] \\
  \lambda_2 &= \delta\left( \gamma - \frac{1}{2} \right) + \gamma \frac{2\gamma-1}{2\gamma+1}\varepsilon + O\left[ \varepsilon^2 \right]
\end{align}
which suggest that the stationary point is stable for $0<\gamma<1/2$.
...
\begin{equation}
  \dot{K}_c = b_c \sqrt{L} s^{1-\gamma}\delta^\gamma \chi^{-\gamma} K_c^{1/2+\gamma} - \delta K_c
	\label{eq:learning_trajectory_ode}
\end{equation}
which can be done by means of separation of variables, resulting in
\begin{equation}
  K_c(t) = \left( \frac{s b_c \sqrt{L}}{\delta} \left( \frac{\delta}{\chi s} \right)^{\gamma}\left( 1-e^{-(t-t_0)/t_c^*} \right) + K_{c0}^{1/2 - \gamma} e^{-(t-t_0)/t_c^*}\right)^{\frac{2}{1-2\gamma}}
	\label{eq:learning_trajectory_solution}
\end{equation}
So, if the clean sector starts from a state with low capital and knowledge, the system approaches its equilibrium approximately on this trajectory i.e. exponentially from below, on a timescale that is given by
\begin{equation}
	t_c^* = \frac{2}{\delta(1-2\gamma)}
	\label{eq_learning_equilibrium_timescale}.
\end{equation}
Assuming $\delta=0.06$ p.a. and $\gamma = 1/8$, the timescale for clean capital accumulation is $t^*_c \approx 44$ years.
...
\begin{wrapfigure}[16]{o}{.45 \textwidth}
        \hspace{-.5 cm}
	\includegraphics[width = .5 \textwidth]{./figures/g_depletion.pdf}
...
In the next series of experiments, I implemented learning in the clean sector as outlined in~\eqref{eq:learning_by_doing}.
