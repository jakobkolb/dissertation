...
% we can leave out Simon2018abmmigration
%Conte2002
%Farmer2009,
%Kirman2010
...
Households are embedded in a social network in which each household $i$ has neighbors $\mathcal{N}(i)$. Inspired by a recent study on heuristic behavior in social learning by \cite{Barkoczi2016}, I implement household behavior in the following way.
...
%Jobst deleted this to save a line: when the household performs the update 
...
Knowing $r(t)$ and $w(t)$, I can now determine which household consumes most after time $\tau$.
...
The analytical approximation of the aggregate savings rate in the stable regime of the agent-based RCK model can be compared to the optimal savings rate in the steady state of the standard RCK model.
...
%old version: When $\tau \to 0$ this has the limit $\rho \to \infty$, which is consistent with the observed myopic behavior, and when $\tau \to \infty$ this has the limit $\rho \to 0$, consistent with the observed far-sighted behavior and the near-optimality of the savings rate.  Note that in this model the effective discounting rate is not a free parameter, but rather it emerges from the collective behavior of the households as determined by the social interaction time $\tau$.
...
%These dynamics are also given as an animation in the Supplementary Material.
...
\begin{wrapfigure}[26]{o}{.6 \textwidth}
  \centering
...
%Jobst: I felt this sounded like the finding rather than the study was motivated by the logic: we find empirically that the critical social interaction time roughly satisfies the proportionality relation 
\begin{equation}
...
%It has long been known that endogenous dynamics are possible in economic models \cite{day1983emergence, 
%Jobst: removed the textbook day1994complex,
%boldrin1992equilibrium, BOLDRIN198626,%boldrin1992sthisces,
%blume1992evolution}, and more recently Beaudry \textit{et al.}\ have shown how limit cycles can emerge in a standard framework where agents are perfect utility maximizers \cite{beaudry2015reviving, beaudry2016putting}. This model here differs in that the endogenous dynamics are caused by the combination of the inherent instability of the RCK model with the heterogeneous and non-optimal behavior of the households.  I believe that this is in part because large values of $\tau$ introduce a lag in optimization {\color{red}``optimization''?}, which in control theory is well-known to cause instability.  I hypothesize that imperfect utility maximization, which makes the system behave as if outcomes do not match expectations, makes the system more prone to oscillations such as business cycles.
